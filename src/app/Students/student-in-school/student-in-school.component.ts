...
import {ColumnProfile, ProfileSettingsComponent} from '../../profile-settings/profile-settings.component';
...
export class StudentInSchoolComponent extends AppComponent {
  ...
  allColumns: ColumnProfile[] = [
    ...
    {name: 'adres_zameldowania', visible: 0, order: 6, label: 'Adres zameldowania'},
    {name: 'numer_telefonu', visible: 0, order: 6, label: 'Numer telefonu'},
  ...
  constructor(private http: HttpClient) {
    ...
    const saved = localStorage.getItem('studentColumnProfile');

    if (saved) {
      this.allColumns = JSON.parse(saved);
    }

    this.getStudents();
  }

  openColumnProfileDialog() {
    const dialogRef = this.dialog.open(ProfileSettingsComponent, {
      width: '500px',
      data: {columns: this.allColumns}
    });

    dialogRef.afterClosed().subscribe((result: ColumnProfile[]) => {
      if (result) {
        this.allColumns = result.map((col, index) => ({...col, order: index}));
        localStorage.setItem('studentColumnProfile', JSON.stringify(this.allColumns));
      }
    });
  }
